...
- **Pomodoro_Session**: A timed work interval associated with a single Work_Category, following the standard Pomodoro technique (25 minutes of work, 5-minute short break, 15-minute long break after 4 sessions).
- **Pomodoro_Manager**: The component responsible for creating, switching, and completing Pomodoro sessions based on detected context switches.
- **Context_Switch**: A detected change in the active Work_Category that persists beyond a configurable debounce threshold.
- **Activity_Log**: A timestamped record of an observed activity stored in the local SQLite database.
...
2. WHEN the Tracker retrieves a window title, THE Tracker SHALL record an Activity_Log entry containing the timestamp, application name, window title, and derived Work_Category.
...
**User Story:** As a user, I want an integrated Pomodoro timer that tracks my focused work intervals, so that I can maintain productive work habits with structured breaks.
...
2. WHEN an Activity_Log entry is written, THE Activity_Log SHALL store the timestamp, application name, window title, Work_Category, and associated Pomodoro_Session identifier.
...
**User Story:** As a user, I want a graphical settings interface to configure email delivery, customize work categories, and manually add tasks to Pomodoro sessions, so that I can personalize FlowTrack without editing configuration files.
...
5. WHEN the user manually creates a Pomodoro task from the system tray menu or settings UI, THE Pomodoro_Manager SHALL start a new Pomodoro_Session with the user-specified category and sub-category label.
6. THE settings window SHALL pre-populate all fields with the current configuration values when opened.
7. WHEN the user saves settings, THE FlowTrack application SHALL persist all changes to the configuration file and apply them to the running application.
