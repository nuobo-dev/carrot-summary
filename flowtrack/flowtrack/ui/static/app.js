...
// --- FIX #2: Timer display only driven by local tick, API just syncs values ---
...
// Status refresh — only syncs reference values, does NOT touch timer display
async function refreshStatus() {
  try {
    ...
    if (d.session) {
      ...
      // Sync the authoritative values — the 1s tick will use these
...
function isTaskTracked(title) {
  if (!currentSession || !title) return false;
  const t = title.toLowerCase();
  const tc = (currentSession.sub_category || currentSession.category || '').toLowerCase();
  const tcat = (currentSession.category || '').toLowerCase();
  return t === tc || t === tcat || tc.includes(t) || t.includes(tc);
}

// --- FIX #3 & #4: Proper drag-drop + inline add-task per bucket ---
let draggedTodoId = null, draggedBucketId = null;

function dragTodo(e, id) {
  draggedTodoId = id; draggedBucketId = null;
  e.dataTransfer.setData('type', 'task');
  e.stopPropagation(); // prevent bucket drag from firing
}
function dragBucket(e, id) {
  draggedBucketId = id; draggedTodoId = null;
  e.dataTransfer.setData('type', 'bucket');
}
...
async function dropOnBucket(e, targetId) {
  ...
  if (type === 'task' && draggedTodoId && draggedTodoId !== targetId) {
    // Move task to this bucket — no confirmation needed
    await fetchJSON(`/api/todos/${draggedTodoId}/move`, {method:'POST',
      headers:{'Content-Type':'application/json'}, body:JSON.stringify({parent_id:targetId})});
    refreshTodos();
  } else if (type === 'bucket' && draggedBucketId && draggedBucketId !== targetId) {
    // Merge buckets — needs confirmation
    if (confirm('Merge these two buckets?')) {
      await fetchJSON('/api/todos/merge', {method:'POST',
        headers:{'Content-Type':'application/json'}, body:JSON.stringify({source_id:draggedBucketId, target_id:targetId})});
      refreshTodos();
    }
  }
...
async function addTaskToBucket(bucketId) {
  ...
  await fetchJSON('/api/todos', {method:'POST', headers:{'Content-Type':'application/json'},
    body:JSON.stringify({title, parent_id: bucketId})});
  inp.value = '';
  refreshTodos();
}

// Todo rendering with inline add-task per bucket
async function refreshTodos() {
  ...
  parents.forEach(p => {
    ...
    children.forEach(c => {
      const ct = !c.done && isTaskTracked(c.title);
      html += `<div class="todo-item child-item ${c.done?'done':''} ${ct?'active':''}"
        ...
        ${ct?'<span class="t-badge tracking">● tracking</span>':''}
    ...
    // Inline add-task for this bucket
...
async function loadActivity(dateStr) {
  ...
  if (!d.categories || !d.categories.length) {
    ...
    document.getElementById('activity-sessions').textContent = '0'; return;
  }
...
function toggleSubs(i) { const el=document.getElementById('subs-'+i),ch=document.getElementById('chev-'+i); if(!el)return; el.style.display=el.style.display==='none'?'':'none'; if(ch)ch.classList.toggle('open'); }
function toggleCollapsed(id,row) { const el=document.getElementById(id); if(!el)return; const show=el.style.display==='none'; el.style.display=show?'':'none'; const n=row.querySelector('.activity-sub-name'); if(n)n.innerHTML=n.innerHTML.replace(/^[▶▼]/,show?'▼':'▶'); }

// Calendar
const DAYS=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
async function renderCalendar(){const grid=document.getElementById('cal-grid'),title=document.getElementById('cal-title'),MN=['January','February','March','April','May','June','July','August','September','October','November','December'];title.textContent=MN[calMonth-1]+' '+calYear;const data=await fetchJSON(`/api/summary/month?year=${calYear}&month=${calMonth}`);let html=DAYS.map(d=>`<div class="cal-header">${d}</div>`).join('');const firstDay=new Date(calYear,calMonth-1,1).getDay(),dim=new Date(calYear,calMonth,0).getDate(),today=new Date().toISOString().split('T')[0];for(let i=0;i<firstDay;i++)html+='<div class="cal-day empty"></div>';for(let d=1;d<=dim;d++){const ds=`${calYear}-${String(calMonth).padStart(2,'0')}-${String(d).padStart(2,'0')}`,has=data.days&&data.days[ds];html+=`<div class="cal-day ${ds===today?'today':''} ${ds===selectedDate?'selected':''} ${has?'has-data':''}" onclick="selectDay('${ds}')" title="${has?data.days[ds].minutes+'m':''}">${d}</div>`;}grid.innerHTML=html;}
...
// Report
async function generateReport(){const s=document.getElementById('report-start').value,e=document.getElementById('report-end').value;if(!s||!e){alert('Select both dates');return;}const out=document.getElementById('report-output');out.className='report-output visible';out.innerHTML='<p style="color:var(--text-tertiary)">Generating...</p>';const d=await fetchJSON(`/api/summary/range?start=${s}&end=${e}`);let html=`<div style="font-weight:600;margin-bottom:8px">${s} to ${e} — ${d.total_time}, ${d.total_sessions} sessions</div>`;d.days.forEach(day=>{if(day.total_time==='0m')return;html+=`<div style="margin-bottom:6px"><div style="font-weight:500;font-size:12px">${day.date} — ${day.total_time}</div>`;day.categories.forEach(c=>{html+=`<div style="font-size:11px;color:var(--text-secondary);padding-left:12px">${c.category}: ${c.time_str}</div>`;});html+='</div>';});out.innerHTML=html;}

// Settings
async function loadConfig(){config=await fetchJSON('/api/config');const p=config.pomodoro||{};document.getElementById('s-work').value=p.work_minutes||25;document.getElementById('s-short').value=p.short_break_minutes||5;document.getElementById('s-long').value=p.long_break_minutes||15;document.getElementById('s-interval').value=p.long_break_interval||4;document.getElementById('s-debounce').value=config.debounce_threshold_seconds||30;document.getElementById('s-poll').value=config.poll_interval_seconds||5;const e=(config.report||{}).email||{};document.getElementById('s-smtp').value=e.smtp_server||'';document.getElementById('s-port').value=e.smtp_port||587;document.getElementById('s-user').value=e.smtp_username||'';document.getElementById('s-pass').value=e.smtp_password||'';document.getElementById('s-to').value=e.to_address||'';}
async function saveSettings(){config.pomodoro={work_minutes:+document.getElementById('s-work').value,short_break_minutes:+document.getElementById('s-short').value,long_break_minutes:+document.getElementById('s-long').value,long_break_interval:+document.getElementById('s-interval').value};config.debounce_threshold_seconds=+document.getElementById('s-debounce').value;config.poll_interval_seconds=+document.getElementById('s-poll').value;if(!config.report)config.report={};config.report.email={smtp_server:document.getElementById('s-smtp').value,smtp_port:+document.getElementById('s-port').value,smtp_username:document.getElementById('s-user').value,smtp_password:document.getElementById('s-pass').value,to_address:document.getElementById('s-to').value,use_tls:true};await fetchJSON('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(config)});const msg=document.getElementById('settings-msg');msg.classList.add('visible');setTimeout(()=>msg.classList.remove('visible'),2000);}

// Actions
async function addTodo(){const inp=document.getElementById('todo-title'),title=inp.value.trim();if(!title)return;await fetchJSON('/api/todos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title})});inp.value='';refreshTodos();}
...
async function startTask(){const cat=document.getElementById('task-cat').value.trim(),sub=document.getElementById('task-sub').value.trim();if(!cat){document.getElementById('task-cat').focus();return;}await fetchJSON('/api/task/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({category:cat,sub_category:sub||cat})});document.getElementById('task-cat').value='';document.getElementById('task-sub').value='';refreshStatus();}

// Timers — only the 1s tick drives the display
