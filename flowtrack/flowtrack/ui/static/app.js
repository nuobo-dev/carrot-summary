...
async function refreshStatus() {
  try {
    ...
    dot.className = 'status-dot' + (d.tracking ? ' on' : '');
    ...
    if (d.session) {
      ...
      lastRemaining = rem; lastRemainingAt = Date.now(); lastTotal = total; lastStatus = s.status;
...
async function dropOnBucket(e, targetId) {
  ...
  if (draggedTodoId && draggedTodoId !== targetId) {
    ...
    if (confirm('Merge these buckets?')) {
...
async function startTask() {
  const cat = document.getElementById('task-cat').value.trim(), sub = document.getElementById('task-sub').value.trim();
  if(!cat){document.getElementById('task-cat').focus();return;}
  await fetchJSON('/api/task/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({category:cat,sub_category:sub||cat})});
  const taskTitle = sub ? cat+': '+sub : cat;
  await fetchJSON('/api/todos',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:taskTitle,category:cat})});
  document.getElementById('task-cat').value=''; document.getElementById('task-sub').value=''; refreshStatus();
}
